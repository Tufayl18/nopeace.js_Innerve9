    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            async function fetchData() {
                try {
                    const estGSTfromcontract = BigInt(await stakingContract.getGSTValueForExchangeValue(ethers.parseEther(ethAmt)));
                    console.log(estGSTfromcontract.toString());
                    setEstGST(parseFloat(ethers.formatEther(estGSTfromcontract)).toFixed(4));
                } catch (e) {
                    console.log(e);
                }
            }
                ...
                </div>
                {
                    !isDepolyer ?
                        ...
                        <><div>
                            ...
                            <div>
                                <button className={styles.WithdrawButton} onClick={fundContract}>Fund Contract 20 GST</button>
                            </div>
                        </>
                }
